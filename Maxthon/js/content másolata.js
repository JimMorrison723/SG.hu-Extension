...
var show_navigation_buttons = {
	...
	activated : function() {
		...
		if(!document.location.href.match('cikkek') && !document.location.href.match('listazas_msg.php')) {
			...
			$('#ext_search').css('background-image', 'url('+ext_url+'img/content/search.png)');
		...
		if(isLoggedIn() || document.location.href.match('listazas_msg.php')) {
			...
			$('#ext_nav_faves').css('background-image', 'url('+ext_url+'img/content/star.png)');
		...
		if (mxstorage.getItem('show_navigation_buttons_night') == 'true') {
			...
			$('#ext_nightmode').css('background-image', 'url('+ext_url+'img/content/lamp' +  state + '.png');
			...
			$('#ext_nightmode').click(function() {
				...
				if (state) {
					...
					//Night mode ON
					$('#ext_nightmode').css('background-image', 'url('+ext_url+'img/content/lampOff.png');
					...
					//Night mode Off
					$('#ext_nightmode').css('background-image', 'url('+ext_url+'img/content/lampOn.png');
...
var lights = {
	...
	topic_switchOn : function() {
		...
		$('body').css('background-image', 'url('+ext_url+'img/content/background.png');
	...
	forum_switchOn : function() {
		...
		$('.td-list2 ').css({ 'background-image' : 'url('+ext_url+'img/content/ext_hsep_bg.png)' });
...
var update_fave_list = {
	...
	activated : function() {
		...
		// Set refresh image
		$('<img src="'+rt.getPrivateUrl('/img/content/refresh.png')+'">').appendTo('#ext_refresh_faves');
	...
	refresh : function() {
		...
		$('#ext_refresh_faves img').attr('src', ext_url +' img/content/refresh_waiting.png');	
		...
		$.ajax({
			...
			success : function(data) {
				...
				// Set 'completed' icon
				$('#ext_refresh_faves img').attr('src', ext_url + 'img/content/refresh_done.png');
				
				// Set back the normal icon in 1 sec
				setTimeout(function() {
					$('#ext_refresh_faves img').attr('src', ext_url + 'img/content/refresh.png' );
				}, 2000);
...
var make_read_all_faves = {
	...
	activated : function() {
		...
		// Append the image
		$('<img src="'+rt.getPrivateUrl('/img/content/makereaded.png">')+'').appendTo('#ext_read_faves');
	...
	makeread : function() {
		...
		if(confirm('Biztos olvasottnak jelölöd az összes kedvenced?')) {
			...
			// Set 'in progress' icon
			$('#ext_read_faves img').attr('src', rt.getPrivateUrl('/img/content/makereaded_waiting.png') );
			...
			var interval = setInterval(function() {
				...
				if(count == counter) {
					...
					// Set 'completed' icon
					$('#ext_read_faves img').attr('src', rt.getPrivateUrl('/img/content/makereaded_done.png') );
			
					// Set normal icon
					setTimeout(function() {
						$('#ext_read_faves img').attr('src', rt.getPrivateUrl('/img/content/makereaded.png') );
					}, 2000);
...
//				textarea_clone.find('a:eq(1)').css({ left : 360 });
		...
		// Add close button
		var close_btm = $('<img src="'+rt.getPrivateUrl('img/content/overlay_close.png')+'" id="ext_close_overlay">').prependTo(textarea_clone).addClass('ext_overlay_close');
		...
		if (comments != undefined) {
			...
			comments.each(function() {
				...
				if($(this).find('.ext_comments_for_me_indicator').length == 0) {
					...
					$(this).css('position', 'relative').append('<img src="'+rt.getPrivateUrl('/img/content/comments_for_me_indicator.png')+'" class="ext_comments_for_me_indicator">');
...
var custom_blocks = {
	...
	setOverlay : function() {
		...
		$('.ext_block').each(function() {
			...
			// Contenthide
			$('<img src="'+rt.getPrivateUrl('/img/blocks/minimalize.png')+'" class="ext_block_button_right">').prependTo(item).click(function(e) {
				e.preventDefault();
				custom_blocks.contentHide( $(this).closest('div').attr('id'), true );
			});

			// Hide
			$('<img src="'+rt.getPrivateUrl('/img/blocks/close.png')+'" class="ext_block_button_right">').prependTo(item).click(function(e) {
				e.preventDefault();
			...
			// Down
			$('<img src="'+rt.getPrivateUrl('/img/blocks/down.png')+'" class="ext_block_button_left">').prependTo(item).click(function(e) {
				e.preventDefault();
				custom_blocks.down( $(this).closest('div').attr('id'), true );
			});

			// Up
			$('<img src="'+rt.getPrivateUrl('/img/blocks/up.png')+'" class="ext_block_button_left">').prependTo(item).click(function(e) {
				e.preventDefault();
				custom_blocks.up( $(this).closest('div').attr('id'), true );
			});						

			// Right
			$('<img src="'+rt.getPrivateUrl('/img/blocks/right.png')+'" class="ext_block_button_left">').prependTo(item).click(function(e) {
				e.preventDefault();
				custom_blocks.right( $(this).closest('div').attr('id'), true );
			});			
			// Left
			$('<img src="'+rt.getPrivateUrl('/img/blocks/left.png')+'" class="ext_block_button_left">').prependTo(item).click(function(e) {
				e.preventDefault();
	...
	activated : function() {
		...
		$('.topichead').closest('table').each(function() {
			...
			if(!$(this).data('events')) {
				...
				{
					//Place info image
				    $(this).addClass('quick_user_info').find('td.right').before('<td class="left" nowrap><img src="'+rt.getPrivateUrl('/img/content/info.png')+'" class="ext_quick_user_info_btn"></td>');
				}
...
var spoiler_button = {
	...
	activated : function() {
		...
		if(mxstorage.getItem('wysiwyg_editor') == 'true') {
			...
			// Add spoiler button
			$('form[name="newmessage"] a:eq(5)').after('<a id="spoilerButton"><img src="'+rt.getPrivateUrl('/img/content/spoiler.png')+'" width="77" height="17" border="0"></a>');
			$('#spoilerButton').css({  'position' : 'absolute', 'left' : 110 , 'cursor': 'pointer'}); // 380
		}
		else {
			// Make macro image smaller
			$('form[name="newmessage"] a:eq(1) img').attr('src', rt.getPrivateUrl('img/content/makro_button.png')).css({ 'width' : '17px' });
