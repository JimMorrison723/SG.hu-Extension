...
var cp = {
	...
	init : function(page) {
		...
		$('<div id="ext_settings_button"><img src="'+ext_url+'img/settings/icon.png" alt=""></div>').appendTo('body');
		...
		html += '<div id="ext_settings_wrapper">';
			...
			html += '<div class="settings_page">';
				html += '<h3>SG Fórum+</h3>';
				html += '<p>Verzió: 3.0.8.2<br></p>';
				html += '<p>Kiadás dátuma: 2014. 06. 26.</p>';
		...
		$('#ext_settings_header li').css({ 'background-image' : 'url('+ext_url+'img/settings/icons.png)' });
		...
		$('.settings_page .button').css({ 'background-image' : 'url('+ext_url+'img/settings/button.png)' });
...
var log = {
	...
	add : function(message, origin) {
		...
		// Add to dataStore
		mxstorage.getItem('debugger_messages') = JSON.stringify(messages);
